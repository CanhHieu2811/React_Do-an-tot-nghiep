...
import { BANNERMOBILE, BANNERCRT, BANNERDEL } from 'src/api/master-data';
import {
  setEqualForm,
  // setPopup,
  setFetchData,
...
import FormThaoTacDuLieu from 'src/template/admin/banner/mobile/form';
...
export default function BannerMobilePages() {
  ...
  const fetchData = useCallback((conditions) => {
    ...
    authGetData({
      ...
      url: `${VITE_REACT_APP_API_MASTER_DATA + BANNERMOBILE}?${buildQueryString(parseParams(conditions))}`,
      onSuccess: (res) => {
        console.log(res);
